...
@test "firewall: dnsmasq DNS guard uses wgclient DNS from UCI" {
  create_vpn_interface wgclient 10.2.0.2/32
  uci_set "network.wgclient.proto" "wireguard"
  uci_set "network.wgclient.dns" "103.86.96.100"

  run "$FIREWALL"
  assert_success

  run grep "add chain inet fw4 dns_vpn_only" /tmp/nft_calls
  assert_success
  ...
  run grep "meta skuid.*453.*jump dns_vpn_only" /tmp/nft_calls
  assert_success
}

@test "firewall: dnsmasq DNS guard falls back to pinned dnsmasq server for WireGuard" {
  create_vpn_interface wgclient 10.2.0.2/32
  uci_set "network.wgclient.proto" "wireguard"
  ...
  run grep "103.86.96.100.*udp dport 53.*accept" /tmp/nft_calls
  assert_success
}

@test "firewall: invalid VPN DNS skips dnsmasq DNS guard" {
  create_vpn_interface wgclient 10.2.0.2/32
  uci_set "network.wgclient.proto" "wireguard"
  ...
  run grep "dns_vpn_only" /tmp/nft_calls
  assert_failure
